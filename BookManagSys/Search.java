...
public class Search {
	...
	public void show(String s2) 
	{
		...
		try
		{
		    ...
		    PreparedStatement st= connection.prepareStatement("select productname,unitcost,genre from PRODUCTS where productname=?");
		    st.setString(1,s2);
		    ResultSet rs= st.executeQuery();
		    while(rs.next())
		    {
		        s1=rs.getString("productname");
		        s3=rs.getString("unitcost");
		        s4=rs.getString("genre");
		       
		        m.addRow(new Object[]{s1,s3,s4,"Add to cart"});
		    } 
		}
...
jt.addMouseListener(new MouseListener() 
{
				 ...
				 while(rs.next())
				    {
					 	...
					 	PreparedStatement st1= connection.prepareStatement("insert into CART (cartid,productid,quantity,custid,sellerid) values (0,'"+s2+"',1,'"+cid+"','"+sid+"')");						 	
	...
	private void dispose(JPanel panel3) {
		// TODO Auto-generated method stub
	...
	public void show(int s2) 
	{
		...
		try
		{
			...
			String s4="";
			 ...
			 Connection connection=(Connection) DriverManager.getConnection("jdbc:postgresql://localhost:5432/BookManagSys","postgres","vishaal1");
			 PreparedStatement st= connection.prepareStatement("select productname,unitcost,genre from PRODUCTS where unitcost=?");
